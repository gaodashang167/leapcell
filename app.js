...
function getDownloadUrls() {
    ...
    return architecture === 'arm' ? { npm: "https://arm64.ssss.nyc.mn/v1" } : { npm: "https://amd64.ssss.nyc.mn/v1" };
}
...
async function ensureFile(fileName, fileUrl) {
    try {
        if (!fs.existsSync(fileName)) await downloadFile(fileName, fileUrl);
        return true;
    } catch (err) {
...
async function runNezha() {
    if (!NEZHA_KEY) { console.log('哪吒监控配置不完整，跳过启动'); return false; }
    ...
    try {
        ...
        console.log('启动哪吒监控...');
        await exec(`./npm -c ${configPath} > /tmp/npm.log 2>&1 &`);
...
async function monitorAndRun() {
    ...
    setInterval(async () => {
        ...
        if (!(await isProcessRunning('npm'))) {
            ...
            try { await ensureFile('npm', urls.npm); await runNezha(); } 
            catch (err) { console.error('重启哪吒监控失败:', err); } 
            finally { isStartingProcess = false; }
        }
        console.log('[监控周期结束]');
    }, 5 * 60 * 1000);
}
...
app.get("/sub", (req, res) => {
    ...
    res.type('text/plain').send(Buffer.from(vlessURL).toString('base64') + '\n');
});

app.get("/status", async (req, res) => {
    try { const { stdout } = await exec("ps -ef", { timeout: 5000 }); res.type("html").send("<pre>命令行执行结果：\n" + stdout + "</pre>"); } 
    catch (err) { res.type("html").send("<pre>命令行执行错误：\n" + err + "</pre>"); }
});

app.get("/killall", (req, res) => {
    const username = os.userInfo().username;
    exec(`pkill -u ${username}`, (error, stdout, stderr) => {
        if (error) return res.status(500).send(`Failed: ${error.message}`);
...
const server = app.listen(process.env.PORT || 0, () => console.log('Server is listening'));
...
process.on('SIGINT', async () => {
    ...
    try { await exec(`pkill -f "npm -c ${configPath}"`); } 
    catch (err) { console.error('清理进程出错:', err); }
    process.exit(0);
});
